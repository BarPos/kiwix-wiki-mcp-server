...
// Create a server instance with all Kiwix tools registered
...
function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  return new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      try {
        resolve(data.length > 0 ? JSON.parse(data) : undefined);
      } catch (error) {
        reject(error);
      }
    });
...
httpServer.listen(MCP_PORT, () => {
  console.error(`Kiwix Wiki MCP Server running at http://localhost:${MCP_PORT}/mcp`);
